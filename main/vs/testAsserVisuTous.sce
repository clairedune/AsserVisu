...
// ------ Camera Pose
posecDesMo_m= [0 0 1 0 0 0 ];              // pose target/object desired
posecMo_m   = [0.1 -0.6 1 0 0 0];              // pose target/object init  
posewMo_m   = [0 0 0 %pi/2 0 0 ];              // pose of the target in the World Frame
Np_m        = 10;                                // horizon lenght
Nc_m        = 1;                                 // command horizon length  
v_m         = [0 0 0 0 0 0]';                    // init velocity

// ------ Constraints definition
xu_m        = [  1 ;  1 ];                 // position max of the a 2D point in the image plane 
...
//funcost_m    = cld_costLocalMire;               // Local prediction sum(e'Qe) + v'Rv
//funpred_m    = ga_predHorLoc2dMire;

//funcost_m    = ga_costLocalMire;                  // Local prediction sum(e'Qe)
//funpred_m    = ga_predHorLoc2dMire;

//funcost_m    = cld_costSQPMire;                 // Np=Nc=1 just to test SQP with sum(e'Qe) + v'Rv
//funpred_m    = ga_predHorLoc2dMire;

//funcostSQP_m = ga_costSQPGlobalMire;
//funpred_m    = ga_predHorGlobalMire;

//funcost_m    = ga_costSQPMire;                  // Np=Nc=1 just to test SQP with sum(e'Qe) + v'Rv
//funpred_m    = ga_predHorLoc2dMire;

funcost_m    = cld_costGlobalMire;               // Global prediction sum(e'Qe) 
funpred_m    = ga_predHorGlobalMire;
...
// -------- OPT
OPT_ERROR     = %F;                               // use error sm-s as a correction
OPT_INEQ      = %F;                               // ineq constraints
OPT_NORMALIZE = %F;                               // normalisation
THRES_CUTLOW  = 1e-5;                             // set to zeros value under this threshold

//OPT_CONTROL   ='SQP';
//OPT_CONTROL = 'AV';
OPT_CONTROL = 'PRED';
OPT_DISPLAY   = %T;                               // display option
OPT_SAVE      = %F;                              // Save option
path_exp = '/home/dune/Documents/Resultats/';
name_exp = 'Pred_gaCost_Np20_NC1'
