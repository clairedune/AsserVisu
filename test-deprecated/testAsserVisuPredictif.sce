...
//------ OPT ------//
OPTZVARYING_m     = %F;          // if true then Z = Z* else Z=Z(t)
...
xl_m      = [ -.22 ; -0.22 ];                     // position min of the a 2D point in the image plane 
bu_m      = [0.25,0.25,0.25,0.25,0.25,0.25]';                // command bounds
bl_m      = -bu_m;
...
//------- Create the object
size_m     = 0.20;                                // size of the target   
oP_m       = mire4points (size_m);                // define target
posewMo_m = [0 0 0 0 %pi 0 ];                     // object pose
wMo_m      = homogeneousMatrixFromPos(posewMo_m); // object pose
...
cP_m   = changeFrameMire(oP_m,cInitMo_m);
