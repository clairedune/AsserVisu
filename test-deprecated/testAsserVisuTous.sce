...
posecMo_m   = [0.05 -0.6 1 0 0 0 ];              // pose target/object init  
...
//funcost_m    = cld_costSQPMire;
//funcost_m    = ga_costSQPMire;
...
while(err > thres_m & iter_m < 1000 )
  ...
  if(OPT_CONTROL == 'PRED')
    ...
    [U_m,smhor_m,Uhor_m] =predControlLocalMire(U0_m,funcost_m,funcst_m,jaccost_m,jaccst_m);
    disp('time')
    toc
  ...
  //elseif(OPT_CONTROL == 'SQP')
    ...
    eps         = 1e-9;      // final norm requirement for d0k
    ...
    v_mb=fsqp(v_m,ipar,rpar,[bl_m,bu_m],ga_costSQPMire,funcst_m,jaccost_m,jaccst_m);
    cost = funcost_m(1,v_mb);
    disp(v_mb')
    disp(cost)
  //--------------------------------------------------------//
  //  Compute the velocity using classical visual servoing  //
  //--------------------------------------------------------//
   ...
   disp('Asservissement visuel classique')
    ...
    v_ma        = computeVelocity(lambda, L_m,s_m-sDes_m);
    v_ma        = v_ma';
    disp(v_ma')
    sm_m       = s_m;
  ...
  //--------------------------- End of velocity computation  //
  if(OPT_NORMALIZE)
     v_ma = normalizeU(v_ma,0.25);
     v_ma = v_ma.*(abs(v_ma)>1e-04);
     disp('prednorm')
     disp(v_ma')
     v_mb = normalizeU(v_mb,0.25);
     v_mb = v_mb.*(abs(v_mb)>1e-04);
     disp('sqpnorm')
     disp(v_mb')
     v_m = normalizeU(v_m,0.25);
     v_m = v_m.*(abs(v_m)>1e-04);
     disp('asservisunorm')
     disp(v_m')
     end
